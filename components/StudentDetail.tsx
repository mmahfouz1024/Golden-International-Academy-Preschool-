...
const StudentDetail: React.FC<StudentDetailProps> = ({ student, readOnly = false, initialDate }) => {
  // Extract language to fix 'Cannot find name language' error
  ...
  const activitiesList = ['Montessori', 'Garden', 'Coloring', 'Art', 'Swimming', 'Puzzle', 'Blocks', 'Songs', 'Etiquette', 'Circle time', 'Learning center', 'P.E'];
  ...
  useEffect(() => {
    ...
    if (allReports[reportKey]) {
      ...
      if (!loadedReport.academic) loadedReport.academic = { religion: [], arabic: [], english: [], math: [] };
      else {
  ...
  const handleSave = () => {
    ...
    const finalReport = { ...report, date: selectedDate, meals: { ...report.meals, ...pendingMealsUpdates }, academic: { ...report.academic, ...pendingAcademicUpdates } };
    try {
      const allReports = getReports();
      const reportKey = `${student.id}_${selectedDate}`;
      ...
      setIsSaved(true);
      addNotification(t('savedSuccessfully'), `${t('newReportMsg')} ${student.name}`, 'success');
      setTimeout(() => setIsSaved(false), 2000);
    } catch (e: any) {
      setSaveError(e.name === 'QuotaExceededError' ? "Storage full!" : "Failed to save report.");
    }
  };
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      // Fixed type error: explicitly cast Array.from result to File[] to avoid 'unknown' inference
      ...
      for (const file of fileArray) {
        try {
          ...
          newPhotos.push(await compressImage(base64));
        } catch (err) { console.error(err); }
      }
      setReport(prev => ({ ...prev, photos: [...(prev.photos || []), ...newPhotos] }));
  ...
  const mealOptions: { value: MealStatus; label: string }[] = [{ value: 'all', label: t('mealAll') }, { value: 'some', label: t('mealSome') }, { value: 'none', label: t('mealNone') }, { value: 'more', label: t('mealMore') }];
      ...
      <div className="space-y-2">
        ...
        {!readOnly && (
          <div className="flex gap-2">
            <input type="text" className={`flex-1 p-2.5 bg-slate-50 rounded-lg border border-transparent focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm ${isLtr ? 'text-left font-sans' : ''}`} dir={isLtr ? 'ltr' : undefined} value={academicInputs[subjectKey]} onChange={e => setAcademicInputs({...academicInputs, [subjectKey]: e.target.value})} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), setReport(prev => ({...prev, academic: {...prev.academic, [subjectKey]: [...(prev.academic?.[subjectKey] || []), academicInputs[subjectKey]]}}), setAcademicInputs({...academicInputs, [subjectKey]: ''})))} placeholder={t('add')} />
            <button onClick={() => (setReport(prev => ({...prev, academic: {...prev.academic, [subjectKey]: [...(prev.academic?.[subjectKey] || []), academicInputs[subjectKey]]}}), setAcademicInputs({...academicInputs, [subjectKey]: ''})))} disabled={!academicInputs[subjectKey].trim()} className="bg-indigo-600 text-white p-2.5 rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-colors shadow-sm"><Plus size={18} /></button>
          </div>
        )}
        <div className="flex flex-wrap gap-2 pt-1">{items.map((item, idx) => (<div key={idx} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm border font-medium bg-white border-slate-200 text-slate-700 shadow-sm`}><span dir={isLtr ? 'ltr' : undefined}>{item}</span>{!readOnly && (<button onClick={() => setReport(prev => ({...prev, academic: {...prev.academic, [subjectKey]: prev.academic?.[subjectKey]?.filter((_, i) => i !== idx) || []}}))} className="text-slate-400 hover:text-rose-500 rounded-full p-0.5 hover:bg-rose-50 transition-colors"><X size={14} /></button>)}</div>))}</div>
      </div>
    );
  };
      ...
      {readOnly && !doesReportExist ? (
        ...
        <>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100"><h3 className="font-bold text-slate-800 mb-5 flex items-center gap-2 text-lg"><Utensils size={20} className="text-orange-600" />{t('meals')}</h3><div className="space-y-5">{[{ key: 'breakfast', label: t('breakfast'), icon: Coffee }, { key: 'lunch', label: t('lunch'), icon: Pizza }, { key: 'snack', label: t('snack'), icon: Apple }].map(meal => {
                const mk = meal.key as any; const dkey = `${mk}Details` as MealDetailsKey;
                return (
                  <div key={meal.key} className="space-y-3 pb-3 border-b border-slate-50 last:border-0"><div className="flex items-center justify-between"><div className="flex items-center gap-2 text-slate-600"><meal.icon size={18} className="text-orange-400" /><span className="text-sm font-bold">{meal.label}</span></div><div className="flex bg-slate-100 rounded-lg p-1 gap-1">{mealOptions.map(opt => (<button key={opt.value} disabled={readOnly} onClick={() => setReport({ ...report, meals: { ...report.meals, [mk]: opt.value } })} className={`px-3 py-1 rounded-md text-[10px] font-bold ${report.meals[mk] === opt.value ? 'bg-white text-orange-600' : 'text-slate-400'}`}>{opt.label}</button>))}</div></div>{!readOnly && (<div className="flex gap-2"><input type="text" className="flex-1 px-3 py-2 bg-slate-50 rounded-xl text-xs outline-none" value={mealInputs[mk]} onChange={e => setMealInputs({...mealInputs, [mk]: e.target.value})} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), setReport(p => ({...p, meals: {...p.meals, [dkey]: [...(p.meals[dkey] || []), mealInputs[mk]]}}), setMealInputs({...mealInputs, [mk]: ''})))} placeholder={t('mealItemPlaceholder')} /><button onClick={() => (setReport(p => ({...p, meals: {...p.meals, [dkey]: [...(p.meals[dkey] || []), mealInputs[mk]]}}), setMealInputs({...mealInputs, [mk]: ''})))} className="bg-indigo-600 text-white p-2 rounded-lg"><Plus size={16} /></button></div>)}<div className="flex flex-wrap gap-2">{(report.meals[dkey] || []).map((item, i) => (<div key={i} className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs bg-orange-50 text-orange-700">{item}{!readOnly && <button onClick={() => setReport(p => ({...p, meals: {...p.meals, [dkey]: p.meals[dkey]!.filter((_, idx) => idx !== i)}}))}><X size={12}/></button>}</div>))}</div></div>
                );
              })}</div></div>
            <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100"><h3 className="font-bold text-slate-800 mb-6 flex items-center gap-2 text-lg"><Droplets size={20} className="text-sky-600" />{t('bathroomLog')}</h3><div className="flex items-center justify-around mb-6">{['urine', 'stool'].map(type => (<div key={type} className="flex flex-col items-center gap-3"><span className="text-sm font-bold text-slate-500 uppercase">{t(type as any)}</span><div className="flex items-center gap-4 bg-slate-50 rounded-2xl p-2"><button disabled={readOnly || (report.bathroom as any)[type] === 0} onClick={() => setReport(prev => ({...prev, bathroom: {...prev.bathroom, [type]: (prev.bathroom as any)[type] - 1}}))} className="w-10 h-10 bg-white rounded-xl shadow-sm"><Minus size={20}/></button><span className="text-2xl font-bold text-indigo-600">{(report.bathroom as any)[type]}</span><button disabled={readOnly} onClick={() => setReport(prev => ({...prev, bathroom: {...prev.bathroom, [type]: (prev.bathroom as any)[type] + 1}}))} className="w-10 h-10 bg-indigo-600 text-white rounded-xl shadow-md"><Plus size={20}/></button></div></div>))}</div></div>
            <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">{renderAcademicSection('religion', t('religion'))}{renderAcademicSection('arabic', t('arabicSubject'))}{renderAcademicSection('english', t('englishSubject'), true)}{renderAcademicSection('math', t('mathSubject'), true)}</div>
          </div>
          {!readOnly && (<div className={`fixed bottom-6 ${language === 'ar' ? 'left-6' : 'right-6'} z-30 flex flex-col gap-3`}><button onClick={handleSave} disabled={isCompressing} className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-8 py-4 rounded-full shadow-2xl font-bold flex items-center gap-3 transition-all hover:scale-105 active:scale-95">{isSaved ? <Check size={24} /> : <Save size={24} />}<span>{isSaved ? t('savedSuccessfully') : t('saveReport')}</span></button></div>)}
        </>
      )}
    </div>
  );
