...
const StudentList: React.FC<StudentListProps> = ({ onStudentSelect }) => {
  ...
  const [studentData, setStudentData] = useState({ name: '', age: '', birthday: '', classGroup: '', parentName: '', phone: '', parentUsername: '', parentPassword: '', parentEmail: '' });
  ...
  const handleOpenModal = (student?: Student) => {
    ...
    setExistingParents(allUsers.filter(u => u.role === 'parent'));
    if (student) {
      setEditingStudent(student); setNewStudentAvatar(student.avatar); setParentMode('new'); 
      const parentUser = allUsers.find(u => u.linkedStudentId === student.id && u.role === 'parent');
      setStudentData({ name: student.name, age: student.birthday ? calculateDetailedAge(student.birthday) : student.age.toString(), birthday: student.birthday || '', classGroup: student.classGroup, parentName: student.parentName, phone: student.phone, parentUsername: parentUser ? parentUser.username : '', parentPassword: '', parentEmail: parentUser?.email || '' });
    } else {
      setEditingStudent(null); setNewStudentAvatar(''); setParentMode('new'); setSelectedParentId('');
      setStudentData({ name: '', age: '', birthday: '', classGroup: classes.length > 0 ? classes[0].name : 'Birds', parentName: '', phone: '', parentUsername: '', parentPassword: '', parentEmail: '' });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSaveStudent = (e: React.FormEvent) => {
    ...
    if (parentMode === 'new' && studentData.parentUsername) {
      ...
      if (isDup) { setError(t('usernameExists' as any)); return; }
    }
    let currentStudentId = editingStudent ? editingStudent.id : Date.now().toString();
    const student: Student = { id: currentStudentId, name: studentData.name, age: parseInt(studentData.age) || 4, birthday: studentData.birthday, classGroup: studentData.classGroup, parentName: studentData.parentName, phone: studentData.phone, status: editingStudent?.status || StudentStatus.Active, attendanceToday: editingStudent?.attendanceToday || false, avatar: newStudentAvatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(studentData.name)}` };
    const updatedList = editingStudent ? students.map(s => s.id === editingStudent.id ? student : s) : [student, ...students];
    setStudents(updatedList); saveStudents(updatedList);

    if (parentMode === 'new' && studentData.parentUsername && studentData.parentPassword) {
        ...
        if (parentUser) {
            ...
            const newUser: User = { id: `u-${Date.now()}`, name: studentData.parentName, username: studentData.parentUsername.trim(), password: studentData.parentPassword, role: 'parent', linkedStudentId: currentStudentId, phone: studentData.phone, email: studentData.parentEmail, avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(studentData.parentName)}`, interests: [] };
  ...
  const filteredStudents = students.filter(s => (s.name.toLowerCase().includes(searchTerm.toLowerCase()) || s.parentName.toLowerCase().includes(searchTerm.toLowerCase())) && (filterClass === 'All' || s.classGroup === filterClass) && (filterAge === 'All' || s.age.toString() === filterAge) && (filterStatus === 'All' || s.status === filterStatus));
      ...
      <div className="bg-white rounded-[2rem] shadow-sm border overflow-hidden">
        <table className="w-full text-left">
            ...
            <tbody className="divide-y">
                {filteredStudents.map(s => (<tr key={s.id} onClick={() => onStudentSelect(s)} className="hover:bg-gray-50 cursor-pointer"><td className="px-6 py-4 flex items-center gap-3"><img src={s.avatar} className="w-8 h-8 rounded-full" /><span>{s.name}</span></td><td className="px-6 py-4">{s.classGroup}</td><td className="px-6 py-4 text-right"><button onClick={(e) => {e.stopPropagation(); handleOpenModal(s);}} className="text-gray-400 hover:text-indigo-600"><Edit2 size={18}/></button></td></tr>))}
            </tbody>
        </table>
      </div>
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-y-auto max-h-[90vh]">
            <div className="p-6 border-b flex justify-between"><h3>{editingStudent ? t('editStudent') : t('addStudentTitle')}</h3><button onClick={() => setIsModalOpen(false)}><X/></button></div>
            <form onSubmit={handleSaveStudent} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="relative"><input type="date" className="w-full p-3 bg-gray-50 rounded-xl opacity-0 absolute inset-0 cursor-pointer" value={studentData.birthday} onChange={handleBirthdayChange} /><div className="p-3 bg-gray-50 rounded-xl text-sm" dir="ltr">{studentData.birthday ? getFormattedDate(studentData.birthday) : t('birthday')}</div></div>
