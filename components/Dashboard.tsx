...
const FOOD_SUGGESTIONS = {
  breakfast: ['Cheese Sandwich', 'Egg Sandwich', 'Jam Sandwich', 'Cereal & Milk', 'Pancakes', 'Falafel', 'Beans'],
  lunch: ['Rice & Chicken', 'Pasta', 'Nuggets & Fries', 'Koshary', 'Burger', 'Pizza', 'Vegetables & Rice', 'Fish'],
  snack: ['Apple', 'Banana', 'Yogurt', 'Biscuits', 'Juice', 'Cake', 'Popcorn', 'Orange']
};
...
const Dashboard: React.FC<DashboardProps> = ({ setCurrentView }) => {
  ...
  const [posts, setPosts] = useState<Post[]>(() => sortPosts(getPosts()));
  ...
  useEffect(() => {
    const loadData = async () => {
        const cloudPosts = await syncPosts();
    ...
    const interval = setInterval(async () => {
        ...
        setPosts(prev => JSON.stringify(prev) !== JSON.stringify(freshPosts) ? sortPosts(freshPosts) : prev);
    }, 5000);
  ...
  const handleTogglePin = (id: string) => {
    const updatedPosts = posts.map(p => p.id === id ? { ...p, isPinned: !p.isPinned } : p);
    ...
    return (
      <div className={`p-4 rounded-2xl border flex flex-col gap-2 ${colorClass}`}>
         <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-2"><div className="p-1.5 bg-white/50 rounded-lg">{React.createElement(icon, { size: 18 })}</div><h4 className="font-bold text-sm opacity-90">{label}</h4></div>
            {canEditMenu && (
               <button onClick={() => setCustomInputMode(prev => ({ ...prev, [mealKey]: !prev[mealKey] }))} className="p-1 hover:bg-white/50 rounded transition-colors text-xs">{isCustom ? <ArrowLeft size={14} /> : <Plus size={14} />}</button>
            )}
         </div>
         {canEditMenu ? (isCustom ? <input type="text" className="w-full bg-white/60 border-none rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-black/10 transition-all" placeholder={t('addCustomDish')} value={dailyMenu[mealKey]} onChange={(e) => handleMenuUpdate(mealKey, e.target.value)} /> : <select className="w-full bg-white/60 border-none rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-black/10 cursor-pointer appearance-none" value={dailyMenu[mealKey]} onChange={(e) => handleMenuUpdate(mealKey, e.target.value)}><option value="">{t('selectDish')}</option>{FOOD_SUGGESTIONS[mealKey].map((item: string) => (<option key={item} value={item}>{item}</option>))}</select>) : <p className="text-lg font-bold truncate">{dailyMenu[mealKey] || <span className="text-sm opacity-50 italic">--</span>}</p>}
      </div>
    );
  };
